...
function doSaveBlogCategory(element) {
    btn = element.innerHTML;
    element.innerHTML = "<i class='fal fa-spinner-third fa-spin'></i>";
    var formData = new FormData($(document.getElementById('frm'))[0]);
    $.ajax({
        ...
        contentType: false,
        type: "post",
        url: MAIN_ADDRESS + "control/doSaveBlogCategory.php",
        data: formData,
        cache: false,
        success: function (data) {
            var obj = JSON.parse(data);
            if (obj.ok == "true") {
                goToUrl(MAIN_ADDRESS + 'panel/blog_category/', false);
            } else {
                showNotification(obj.result);
                element.innerHTML = btn;
            }
...
function deleteBlogCategory(element, id) {
    showAlert('حذف دسته بندی', 'آیا از حذف دسته بندی مطمئن هستید؟', 'بله', function () {
        btn = element.innerHTML;
        element.innerHTML = "<i class='fal fa-ellipsis-h fail'></i>";
        closeAlert();
        $.ajax({
            type: "post",
            url: MAIN_ADDRESS + "control/deleteBlogCategory.php",
            data: ({id: id}),
            cache: false,
            success: function (data) {
                var obj = JSON.parse(data);
